...
from models.fedgnn_lineartransform import *
...
if __name__ == "__main__":
    ...
    if parser.local:
        ...
        model_name = "traffic-la_global_"

        if not parser.MLP:
            if parser.KnownGraph:
                model_name = model_name + "knowngraph_"
            else:
                model_name = model_name + "learngraph_"
                if parser.isGumbel:
                    model_name = model_name + "Gumbel_"
                else:
                    model_name = model_name + "ICDF_"
        else:
            model_name = model_name + "MLP_"

        if parser.permute:
            model_name = model_name + "permute_"
        model_name = model_name +"lr_" + str(lr) + "_dim"+str(hidden_dim) + ".model"

        if parser.MLP:
            print("Use GNN or MLP? \t MLP.")
            ...
            print("Use GNN or MLP? \t GNN.")
            print("KnownGraph:", parser.KnownGraph)
            synthesizer = Trainer(input_dim, no_event, no_series, workspace.nodes, hidden_dim=hidden_dim, lr=lr,
                                  permute=parser.permute, isGumbel=parser.isGumbel, model_name=model_name, permute_reg=parser.reg, tau=tau)  # must use the same hidden dim as each node

            if parser.isload:
                synthesizer.load_state_dict(torch.load('./results/savedmodels/'+model_name,map_location=device),strict=False)
                permutation_param = synthesizer.permutation_module.permutation_M
                permutation_matrix = torch.stack([synthesizer.permutation_module.sinkhorn(permutation_param[i]) for i in range(no_series)], 0)
                np.savez('./results/middle_results/'+model_name+'.npz', permutation_param = permutation_param.detach().numpy(), permutation_matrix = permutation_matrix.detach().numpy(), graph=torch.sigmoid(synthesizer.generator.prob).detach().numpy())
            else:
                if not parser.KnownGraph:
                    synthesizer.fit(global_data, n_iter = n_iter, period = 5, n_train_sample = n_train_sample, X_val = X_val[:, :, :no_series, :], Y_val = Y_val, Xt = X_test[:, :, :no_series, :], Yt = Y_test, n_test_sample = n_test_sample)
                else:
                    synthesizer.fit(global_data, graph = graph.to(device), n_iter = n_iter, period = 5, n_train_sample = n_train_sample, X_val = X_val[:, :, :no_series, :], Y_val = Y_val, Xt = X_test[:, :, :no_series, :], Yt = Y_test, n_test_sample=n_test_sample)
