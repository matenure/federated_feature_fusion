from models.gnn_transformer import *
...
class InferenceNet(nn.Module):  # accept the stacked embedding collected from N sensors (N = no_sensor)
    def __init__(self, input_dim, no_event, no_sensor, hidden_dim = 8, with_graph = True, istrain=True):
        ...
        self.predictor = Predictor(self.input_dim, self.no_event, self.hidden_dim, self.no_sensor, withGraph=self.withGraph).to(device)
        self.models = nn.ModuleList([self.identifier, self.predictor]).to(device)
        self.istrain = istrain

    def forward(self, X, normalize = True):  # X is (batch, no_sensor, input dim) -- A is an adjacent matrix (no_sensor, no_sensor)
        ...
        output = self.predictor(output, normalize = normalize)  # output is (batch, no_event)
        return output
...
class Trainer(nn.Module):
    def __init__(self, input_dim, no_event, no_sensor, nodes, hidden_dim = 16, lr=0.01, batch_size=100, permute=False,with_graph = True, isGumbel=False, permute_reg=False, tau=0.1, model_name="global.model"):
        ...
        self.model = nn.ModuleList([self.predictor, self.permutation_module]).to(device)
    ...
    def ask(self, X, start=True):  # X is (sensor, window size, batch, input dim) -- probably cache all these in advance
        if self.permute and start: # adding the parameter start to make training faster in the beginning
            self.permutation_M_bin = [self.permutation_module.sinkhorn(self.permutation_module.permutation_M[i]) for i in range(self.no_sensor)]
            H = torch.stack([torch.matmul(self.nodes[i].summarize(X[i]).detach(), self.permutation_M_bin[i]) for i in range(len(self.nodes))]).to(
                device)  # H is (no_sensor, batch, hidden dim)
        else:
            H = torch.stack([self.nodes[i].summarize(X[i]).detach() for i in range(len(self.nodes))]).to(device)  # H is (no_sensor, batch, hidden dim)

        H = H.permute(1, 0, 2)  # H is (batch, no_sensor, hidden dim)
        return H

    def loss(self, X, Y):  # X is (batch, no_sensor, hidden_dim)
    ...
    def predict(self, X, graph = None, mask=None, n_sample = 10, normalize = False):  # X is (batch, no_sensor, hidden_dim)
        with torch.no_grad():  # prediction phase -- no need to propagate gradient
            pred = self.predictor(X, normalize = normalize)
    ...
    def load_test(self, Xt, Yt, graph = None, n_test_sample = 10, mask_test=None):
        Xt_sum = self.ask(Xt.permute(2, 1, 0, 3))
        test_acc, test_f1, test_roc, test_pr, _ = self.pred_scores(Xt_sum, Yt, graph=graph, mask=mask_test,
                                                                   n_sample=n_test_sample)
        print(" ACC (test) = {} | F1 (test) = {} | RocAUC_macro = {} | PRAUC = {}".format(
    ...
    def fit(self, dataset, graph = None, n_iter = 100, n_train_sample = 1, n_test_sample = 10, verbal = True, period = 10, X_val = None, Y_val = None, mask_val = None, Xt = None, Yt = None, mask_test=None):
        ...
        for i in range(1, n_iter + 1):
            if graph is not None:
                ...
                n_train_sample = 1
            ave, n_batch = 0.0, 0.0
            # if i >20 and self.permute and self.permutation_module.exp: # do not update permutation parameter after 20 epochs
            #     del self.optimizer.param_groups[0]
            #     self.permutation_module.permutation_M.requires_grad = False
            ...
            for (X, Y, mask_train) in dataset:  # dataset is a data loader object -- Y is (batch, no_event) -- X is (batch, window size, sensor, input dim)
                ...
                X = self.ask(X, i>=start_permute_epoch)  # X is (batch, no_sensor, hidden_dim)
                ...
                if self.permute:
                    func += 0.01*self.permute_entropy()
                ave += func.item()  # accumulate batch loss
                n_batch += 1.0  # accumulate no. batch
                func.backward(retain_graph = False)

                # #### output middle results: (gradient information)
                # if i <51:
                #     finalgrad_total_norm = 0.0
                #     for name,param in self.model.named_parameters():
                #         if param.requires_grad:
                #             if param.grad is not None:
                #                 param_norm = param.grad.data.norm(p=2)
                #                 finalgrad_total_norm += param_norm.item() ** 2
                #     finalgrad_total_norm = finalgrad_total_norm ** 0.5
                #     finalgrad_theta_norm = self.generator.prob.grad.data.norm(p=2)
                #     final_grads.append([finalgrad_theta_norm, finalgrad_total_norm])
            ...
            if verbal is True and i % period == 0:
                if Xt is None or Yt is None:
                    ...
                    print("Update Test Res at Training Epoch {} | ACC (test) = {} | F1 (test) = {} | RocAUC_macro = {} | PRAUC = {}".format(
                        i, test_acc, test_f1, test_roc, test_pr))
        ...
        #     XX = self.generator.prob.detach().clamp(min=0.0001, max=0.9999).requires_grad_(True)
        #
        #     YY = self.generator.sample_A(XX, 1)
        #     ZZ = YY.sum()
        #     if i <51:
        #         zgr = torch.autograd.grad(ZZ, XX)
        #         for k in range(self.no_sensor):
        #             for j in range(k):
        #                 # gr = torch.autograd.grad(YY[k, j, 0], XX, retain_graph=True)
        #                 grads.append([XX[k, j].data.cpu().numpy(), zgr[0][k, j].cpu().numpy()])
        #
        #         if i%5==0:
        #             np.savez('./results/middle_results/' + self.model_name + '_grads.npz', theta_grads=np.stack(grads, 0), final_grads = np.stack(final_grads,0))
        # np.savez('./results/middle_results/' + self.model_name + '_grads.npz', theta_grads=np.stack(grads, 0),
        #          final_grads=np.stack(final_grads, 0))
    def forward(self):
        pass
